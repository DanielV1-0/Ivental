          ...
          {product.videoURL && (
            <View style={styles.videoActions}>
              <TouchableOpacity
                ...
                <Icon
                  name={showVideo ? 'image-outline' : 'video-outline'}
                  size={18}
                />
              </TouchableOpacity>
              {showVideo && (
    ...
    if (product) {
      return (
        <View style={Styles.paddingHorizontal16}>
          ...
          <SizedBox height={8} />
          <View style={Styles.row}>
            <Text typography="h3" weight="bold" style={Styles.flex}>
              {product?.title}
            </Text>
            {product?.status && (
              <View
            ...
            return (
              <View key={`${item?.id}${index}`}>
                <ProductItem
                  ...
                  onPress={() => onPressProduct(item)}
                />
                {index < product?.related.length - 1 && (
                  <View style={Styles.paddingVertical4}>
                    <Divider />
                  </View>
                )}
              </View>
      ...
      return (
        <View style={Styles.nativeRightButton}>
          ...
          <IconButton
            ...
            onPress={onFavorite}
            style={{backgroundColor: theme.colors.border + Opacity[60]}}>
            <Icon name={product?.favorite ? 'heart' : 'heart-outline'} />
          </IconButton>
          <SizedBox width={8} />
          <IconButton
